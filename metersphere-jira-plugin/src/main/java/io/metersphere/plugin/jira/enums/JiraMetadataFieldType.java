...
public enum JiraMetadataFieldType {
     ...
     * INPUT(特殊自定义字段类型: url)
     */
    INPUT(Set.of("textfield", "url", JiraMetadataField.SUMMARY_FIELD_NAME), PlatformCustomFieldType.INPUT.getType()),
    /**
     * MULTIPLE_INPUT
     */
    MULTIPLE_INPUT(Set.of("labels"), PlatformCustomFieldType.MULTIPLE_INPUT.getType()),
    /**
     * 文本框
     */
    TEXTAREA(Set.of("textarea"), PlatformCustomFieldType.TEXTAREA.getType()),
    /**
     * 单选复选框
     */
    RADIO_BUTTON(Set.of("radiobuttons"), PlatformCustomFieldType.RADIO.getType()),
    /**
     * 浮点数
     */
    FLOAT(Set.of("float"), PlatformCustomFieldType.FLOAT.getType()),
    /**
     * 日期(特殊自定义字段类型: datepicker)
     */
    DATE(Set.of("date", "datepicker"), PlatformCustomFieldType.DATE.getType()),
    /**
     * 日期时间
     */
    DATETIME(Set.of("datetime"), PlatformCustomFieldType.DATETIME.getType()),
    /**
     * SELECT(特殊自定义字段类型: version)
     */
    SELECT(Set.of("version", "select", "priority"), PlatformCustomFieldType.SELECT.getType()),
    /**
     * MULTIPLE_SELECT(特殊自定义字段类型: multiversion)
     */
    MULTIPLE_SELECT(Set.of("multiselect", "multiversion", "components", "fixVersions", "versions"), PlatformCustomFieldType.MULTIPLE_SELECT.getType()),
    /**
     * 级联选择(后续如果需特殊处理选项值, 可移除)
     */
    CASCADING_SELECT(Set.of("cascadingselect"), PlatformCustomFieldType.CASCADE_SELECT.getType()),
    /**
     * 富文本
     */
    RICH_TEXT(Set.of(JiraMetadataField.DESCRIPTION_FIELD_NAME, "environment"), PlatformCustomFieldType.RICH_TEXT.getType());
